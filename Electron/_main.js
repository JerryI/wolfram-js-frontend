...
const subscribe = (server, handler = () => {}, success = () => {}) => {
    let running = false;
...
const storeStartupSettings = (text) => {
    fs.writeFile(path.join(installationFolder, 'path.ini'), text, function(err) {
        if (err) throw err;
    });    
}

const checkStartupSettings = () => {
    if (!fs.existsSync(path.join(installationFolder, 'path.ini'))) return false;
    return fs.readFileSync(path.join(installationFolder, 'path.ini'), 'utf8');
}

const askForPath = (server_, window) => {
    var storedPath;
    if (storedPath = checkStartupSettings()) {
        server = exec(storedPath, { cwd: workingDir });

        subscribe(server, (log) => {
            if (noscript.exec(log) || nofile.exec(log)) {
                sender('FAILED there is no wolframscript in PATH!\n\n', '\x1b[32m');
            ...
            if (nolicense.exec(log)) {
                activateWL(server, path, () => {
                    ...
                    subscribe(server, (log) => {
                        if (nolicense.exec(log)) {
                            sender('\nThere are some problems with your license... Try to run wolframscript in the terminal\n', '\x1b[32m');
    ...
    sender('Please type the path to Wolframscript manually in the box below', '\x1b[32m');
    ...
    makePromt((path) => {
        ...
        subscribe(server, (log) => {
            if (noscript.exec(log) || nofile.exec(log)) {
                sender('FAILED Please enter again!\n\n', '\x1b[32m');
            ...
            /* on success */
            storeStartupSettings(`"${path.replace(/(\n|\r|\r\n)/gm, "")}"`);
            sender('Startup settings are stored at AppData directory in path.txt file', '\x1b[32m');
        });
...
const startServer = (win) => {
    ...
    server = exec('wolframscript5', { cwd: workingDir });
